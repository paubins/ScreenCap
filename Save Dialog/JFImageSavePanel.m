...
- (id)init
{
    ...
    if (self) {
        ...
        self.title      = @"Save Video Journal";
        self.imageType  = kUTTypeMPEG4;
...
//    [self.savePanel setAccessoryView:self.accessoryView];
    ...
    [self.savePanel setAllowsOtherFileTypes:NO];
...
- (NSInteger)runModalForImage:(NSURL *)image error:(NSError **)error
{
    ...
    if (result == NSFileHandlingPanelOKButton) {
        NSError *error = nil;
        [[NSFileManager defaultManager] copyItemAtPath:image.path toPath:savePanel.URL.path error:&error];
        
        [NotificationController notificationMessage:@"File saved!" informative:@"File has been saved!" isSuccess:YES];
    }
